...
#98 adds the first narrow Concept/category-like structural result. Using the already-merged probe-relative equivalence relation, the same retained carrier values fall into different induced classes under different declared probe families. Thus the retained carrier alone does not determine one unique category partition. In this finite scope Concept-like class membership is reconstructed from carrier values + explicit probe family + induced equivalence/quotient construction; extensionally identical probe families preserve the result, and no Concept object or category table is required. This does not establish semantic concepts, understanding, abstraction learning, concept acquisition, language grounding, or a privileged ontology of categories.
