...
#97 tests a narrow Concept-like category role without introducing a Concept object. One fixed response profile over `Item × Probe` makes two analytically distinct items equivalent under one declared probe and distinguishable under another. For each fixed probe, response equality is an equivalence relation; pointwise-equal profiles preserve every such judgment, and a decorative Concept flag changes nothing. Thus this operational category role is reconstructed from the explicit response profile + probe, and the induced partition is probe-relative. Semantic concepts, understanding, language/symbol grounding, abstraction learning, prototypes, ontology learning, and concept-formation history remain separate.
