...
3. **Check the observation bridge.** For a test-family inference, establish \(\operatorname{Adm}_r(A)\). If an admitted test lacks the declared observation-to-target factorization, family-level separation is not licensed by the exact model.
