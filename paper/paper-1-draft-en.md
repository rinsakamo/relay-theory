...
4. **Check independent warrant and representation structure.** State the scientific reason for accepting \(r\), the relevant factorization, the selected family, and the representation-level relations, functions, or metrics used by the inference (denoted \(\mathcal S\) below). These commitments must be non-self-licensing and fixed before the discriminating comparison rather than chosen to preserve a preferred result.
