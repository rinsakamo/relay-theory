...
3. **A reusable individuation-inference audit.** The paper turns the structural conditions into a protocol that checks within-fiber leakage, observation-to-target bridging, independent warrant, structure-preserving re-encoding, assignment sensitivity, and regime overreach.

4. **A machine-checked dependency analysis with cross-domain cases.** Machine-checked proofs verify quotient descent and family-level separation. Bibliographic records provide the motivating case, molecular counting provides a cross-domain scientific stress test, and evidence synthesis shows how the same discipline scales to study- and claim-level analysis.
...
### 2.2 Structural target factorization and quotient descent
...
and let \(\pi_r:P\to P/{\sim_r}\) be the quotient map that identifies every pair of representations assigned to the same target. Call \(f\) **fiber-invariant** when
...
1. **Declare the target model and scope.** Specify \(P\), \(T\), the proposed assignment \(r\), the selected test family \(A\), and the strength of the intended conclusion. If these are not fixed, the audit is underdetermined.
2. **Check quotient descent.** Ask whether each candidate representation-level discriminator is constant on the fibers of \(r\). A within-fiber difference is a structural failure for target-level use.
3. **Check the observation bridge.** For a pre-specified test \(q\), establish \(B_r(q)\). If the analysis may select an arbitrary separating witness from \(A\), establish the stronger \(\operatorname{Adm}_r(A)\) or supply an independently fixed witness-selection rule.
4. **Check independent warrant and representation structure.** State the scientific reason for accepting \(r\), the relevant factorization, the selected family, and the representation-level relations, functions, or metrics used by the inference (denoted \(\mathcal S\) below). These commitments must be non-self-licensing. For confirmatory use they should be fixed before the discriminating comparison; if they are learned adaptively, the selection rule becomes part of the inference and must itself be audited.
5. **Run the counterfactual and scope checks.** Test invariance under structure-preserving re-encodings; if assignment perturbations change the conclusion, identify the grounded relation that makes that sensitivity evidentially relevant. Finally, keep the conclusion no stronger than the selected observational or interventional regime supports.

The audit has three useful dispositions. A violated quotient or factorization condition is a **structural failure**. Structural coherence without adequate independent warrant is **scientifically underdetermined**. A claim that passes both layers is a **scoped pass**: it is licensed relative to the declared target model and regime, not established as unrestricted numerical identity.

## 3. Scientific Stress Test: Molecular Counting with Unique Molecular Identifiers

### 3.1 Why many reads need not mean many source molecules

A **unique molecular identifier (UMI)** is a short sequence tag introduced before amplification so that downstream observations can retain information about the template molecule from which they arose. It is not an intrinsic identity carried by the molecule; it is a protocol-generated tag whose evidential value depends on how it is assigned, propagated, observed, and modeled. Kivioja et al. (2012) demonstrated how such tags can support absolute molecule counting.

**Polymerase chain reaction (PCR)** is an amplification process that can generate many copies from a template molecule. Those copies may in turn produce multiple sequencing read records. Therefore
