...
For the present case, the cleanest target domain is not an abstract molecule identity read directly from a barcode. Let \(T\) contain \textbf{tagged template molecules after UMI assignment and before PCR amplification}.
...
\begin{quote}
\textbf{When an individuation claim depends on a formal difference, expose the target factorization and the scientific reason for accepting it; then test the claim only under an explicitly admitted observational or interventional regime.}
\end{quote}
