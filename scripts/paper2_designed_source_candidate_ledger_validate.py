...
def validate_candidate(record: dict[str, Any], context: str) -> tuple[str, str]:
...
def validate(data: Any, geometry: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, (record, slot) in enumerate(zip(records, expected)):
        ...
        if data["state"] == TEMPLATE_STATE:
            ...
            ident = validate_candidate(record, context)
...
def expect_invalid(data: dict[str, Any], geometry: dict[str, Any], label: str) -> None:
    try:
        validate(data, geometry)
    except ValidationError:
        return
    raise AssertionError(f"{label}: unexpectedly validated")


def self_test(ledger_path: Path, geometry_path: Path) -> None:
    ...
    x["records"].pop()
    expect_invalid(x, geometry, "missing ranked position")

    x = copy.deepcopy(base)
    x["records"][0]["candidate_rank"] = 2
    expect_invalid(x, geometry, "rank drift")

    x = copy.deepcopy(base)
    x["records"][0]["sampling_stratum"] = "Learning"
    expect_invalid(x, geometry, "slot/stratum drift")

    x = copy.deepcopy(base)
    x["phi_consumed"] = True
    expect_invalid(x, geometry, "Phi-before-ledger leak")

    x = copy.deepcopy(base)
    x["state"] = FROZEN_STATE
    expect_invalid(x, geometry, "frozen ledger with unassigned candidates")
    ...
    expect_invalid(x, geometry, "duplicate source identity")

    x = copy.deepcopy(full)
    x["records"][0]["registry_reference"] = "missing"
    expect_invalid(x, geometry, "missing registry linkage")

    x = copy.deepcopy(full)
    x["records"][0]["candidate_rationale"] = "Selected because Phi says EQUIVALENT."
    expect_invalid(x, geometry, "outcome-dependent rationale")

    x = copy.deepcopy(full)
    x["records"][0]["source_access_class"] = "INACCESS"
    x["records"][0]["read_status"] = "NOT_READ"
    expect_invalid(x, geometry, "inaccessible frozen candidate")
...
def main() -> None:
    ...
    validate(data, geometry)
    if args.self_test:
        self_test(args.ledger, args.geometry)
    else:
