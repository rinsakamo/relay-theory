#!/usr/bin/env python3
"""Own the Paper 2 #184 Normal -> SystemOne transaction.

This entry point is deliberately separate from the consumed #147 runner.  It
...
RECEIPT_VERSION = "paper2-two-pass-extraction-transaction-receipt-v1"
SUMMARY_VERSION = "paper2-two-pass-extraction-transaction-summary-v1"
...
def verify_authorization(
    *,
    ...
    if comment_id is None or comment_id <= 0:
        fail("#184 authorization comment ID is required")
    if status != "SCIENTIFIC_TRANSACTION_AUTHORIZED_UNSPENT":
        fail("#184 authorization status is missing or not exact")
    ...
    blob = run_text(
        [
            "git",
            "rev-parse",
            "HEAD:scripts/paper2_extraction_two_pass_llama_cpp_transaction.py",
        ],
        cwd=repo_root,
    ).strip()
...
def execute_bundle(
    *,
    ...
    row["bundle_artifact"] = freeze_artifact(root, row_root / "source-bundle.json", bundle_raw)
    ...
    # Preserve only the claim path in the in-memory execution record; the
    # response/decision bytes remain in the evidence root and are hash-linked.
    row["claim_ir_value"] = claim
    row.pop("claim_ir_value")
...
def compare_completed_claims(
    *,
    ...
    for bundle in EXPECTED_BUNDLES:
        claim_a_path = root / rows_by_pass["A"][bundle]["claim_ir"]["path"]
        claim_b_path = root / rows_by_pass["B"][bundle]["claim_ir"]["path"]
        claim_a = load_json(claim_a_path)
        claim_b = load_json(claim_b_path)
        ...
        differing_fields = {
            ...
            "node_structure": not comparison["node_structure_exact_under_id_renaming"],
            "relation_structure": not comparison["relation_structure_exact_under_id_renaming"],
            "node_descriptions": not comparison["node_descriptions_exact_after_normalization"],
            "relation_descriptions": not comparison["relation_descriptions_exact_after_normalization"],
        }
        graph_disagreement = bool(
            differing_fields["node_structure"] or differing_fields["relation_structure"]
        )
        adjudication_required = not comparison["overall_exact_structural_equivalence"]
        report_rows.append(
            {
                "bundle_id": bundle,
                ...
                "claim_ir_valid_a": True,
                "claim_ir_valid_b": True,
                "claim_ir_sha256_a": rows_by_pass["A"][bundle]["claim_ir"]["sha256"],
                "claim_ir_sha256_b": rows_by_pass["B"][bundle]["claim_ir"]["sha256"],
                "comparison": comparison,
                "field_wise_differences": differing_fields,
                "source_only_adjudication": {
                    "required": adjudication_required,
                    "status": "REQUIRED_SOURCE_ONLY" if adjudication_required else "NOT_REQUIRED",
                    "basis_visible": False,
    ...
    comparisons = [item["comparison"] for item in report_rows]
    scope_counts = {
        field: sum(
            1
    ...
    aggregate = {
        ...
        "exact_structural_agreement_count": sum(
            1 for item in comparisons if item["overall_exact_structural_equivalence"]
        ),
        ...
        "exact_full_agreement_count": sum(
            1 for item in comparisons if item["overall_exact_full_equivalence"]
        ),
        "same_claim_type_count": sum(1 for item in comparisons if item["same_claim_type"]),
        "same_modality_count": sum(1 for item in comparisons if item["same_modality"]),
        "scope_agreement_count_by_field": scope_counts,
        "source_span_exact_count": sum(
            1 for item in comparisons if item["source_span_agreement"]["exact"]
        ),
        "node_structure_exact_count": sum(
            1
            for item in comparisons
            if item["node_structure_exact_under_id_renaming"]
        ),
        "relation_structure_exact_count": sum(
            1
            for item in comparisons
            if item["relation_structure_exact_under_id_renaming"]
        ),
        "source_only_adjudication_count": sum(
            1
            for item in report_rows
            if item["source_only_adjudication"]["required"]
        ),
        "decomposition_visible_graph_structure_disagreement_count": sum(
            1
            for item in report_rows
            if item["decomposition_visible_graph_structure_disagreement"]
        ),
    }
    report = {
        "schema_version": "paper2-two-pass-ab-comparison-v1",
        "owner_issue": OWNER_ISSUE,
        "status": "BASIS_BLIND_COMPARISON_BEFORE_DECOMPOSITION",
        "basis_visible": False,
        "decomposition_outcome_visible": False,
        "rows": report_rows,
        "aggregate": aggregate,
    }
    report_path = root / "ab-comparison.json"
    report["report_artifact"] = freeze_artifact(
        root, report_path, canonical_json_bytes(report)
    )
    # The report's own artifact reference is intentionally not included in the
    # hashed report bytes, avoiding a self-referential digest.
    return report
...
def run_transaction(
    *,
    ...
    try:
        ...
        extractor_identity = "paper2-two-pass-llama-cpp-transaction-v1"
        extractor_revision = (
        ...
        if counters["claim_ir_valid"] != 2 * len(EXPECTED_BUNDLES):
            fail("not all ten ClaimIR outputs validated")
...
def self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="relaytheory-184-selftest-") as temp:
        ...
        for name, plan, expected_systemone in (
            ...
            ("candidate-failure", {"collapse_anchors": {"A": ["B0002"]}}, 2),
        ):
    ...
    print("PAPER2_TWO_PASS_LLAMA_CPP_TRANSACTION_V1_SELFTEST_PASS")


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the authorized Paper 2 #184 two-pass transaction")
    ...
    if args.self_test:
        try:
            ...
            print(f"PAPER2_TWO_PASS_LLAMA_CPP_TRANSACTION_V1_SELFTEST_FAIL: {exc}")
            return 2
